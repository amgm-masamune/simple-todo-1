...
Deno.test("未完了→進行中 で startedAt に UNSEPCIFIED を指定しても変更できる", () => {
  // Given
  const task = Task.create({
    id: TASK_ID,
    title: "task",
...
Deno.test("未完了→完了 で startedAt・completedAt に UNSEPCIFIED を指定しても更新できる", () => {
  // Given
  const task = Task.create({
    id: TASK_ID,
    title: "task",
...
Deno.test("未完了→キャンセル で cancelledAt に UNSEPCIFIED を指定しても更新できる", () => {
  // Given
  const task = Task.create({
    id: TASK_ID,
    title: "task",
...
// TODO: completedAt 未指定・UNSEPCIFIED指定
