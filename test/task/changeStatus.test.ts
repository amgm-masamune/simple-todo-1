...
Deno.test("未完了→進行中 で startedAt に NOT_SPECIFIED を指定しても変更できる", () => {
  // Given
  const task = Task.create({
    id: TASK_ID,
    title: "task",
  ...
  // When
  const now = DATE_2;
  const startedAt = NOT_SPECIFIED;
  const updated = task.changeStatus("in-progress", { startedAt }, now);

  // Then
  assertEquals(updated.status, "in-progress");
  assertEquals(updated.startedAt, NOT_SPECIFIED);
  assertEquals(updated.updatedAt, now);
});
...
Deno.test("未完了→完了 で startedAt・completedAt に NOT_SPECIFIED を指定しても更新できる", () => {
  // Given
  const task = Task.create({
    id: TASK_ID,
    title: "task",
  ...
  const now = DATE_2;
  const startedAt = NOT_SPECIFIED;
  const completedAt = NOT_SPECIFIED;
  const updated = task.changeStatus("completed", { startedAt, completedAt }, now);

  // Then
  assertEquals(updated.status, "completed");
  assertEquals(updated.startedAt, NOT_SPECIFIED);
  assertEquals(updated.completedAt, NOT_SPECIFIED);
  assertEquals(updated.updatedAt, now);
});
...
Deno.test("未完了→キャンセル で cancelledAt に NOT_SPECIFIED を指定しても更新できる", () => {
  // Given
  const task = Task.create({
    id: TASK_ID,
    title: "task",
  ...
  // When
  const now = DATE_2;
  const cancelledAt = NOT_SPECIFIED;
  const updated = task.changeStatus("cancelled", { cancelledAt }, now);

  // Then
  assertEquals(updated.status, "cancelled");
  assertEquals(updated.cancelledAt, NOT_SPECIFIED);
  assertEquals(updated.updatedAt, now);
});
...
// TODO: completedAt 未指定・NOT_SPECIFIED指定
