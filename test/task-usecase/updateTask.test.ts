...
Deno.test("進行中→完了 の状態変更で startedAt を指定しない場合、変更されない(nullにはならない)", async () => {
  ...
  // Given
  const inProgressTask = await createTaskUseCase.execute({ title: "task", status: "in-progress", due: null, startedAt: DATE_1 });
...
Deno.test("完了→進行中 の状態変更で startedAt を指定しない場合、変更されない(nullにはならない)", async () => {
  ...
  // Given
  const inProgressTask = await createTaskUseCase.execute({ title: "task", status: "completed", due: null, startedAt: DATE_1, completedAt: DATE_2 });
