...
Deno.test("未着手→進行中 の状態変更で startedAt を指定すると、その値で進行中になる。", async () => {
  ...
  // Given
  const unstartedTask = await createTaskUseCase.execute({ title: "task", status: "unstarted", due: NOT_SPECIFIED });
...
Deno.test("未着手→進行中 の状態変更で、startedAt の指定がない(null指定もない)場合は変更できない", async () => {
  ...
  /// Given
  const task = await createTaskUseCase.execute({ title: "task", status: "unstarted", due: NOT_SPECIFIED });
...
Deno.test("進行中→完了 の状態変更で startedAt を指定しない場合、変更されない(nullにはならない)", async () => {
  ...
  // Given
  const inProgressTask = await createTaskUseCase.execute({ title: "task", status: "in-progress", due: NOT_SPECIFIED, startedAt: DATE_1 });

  // When
  const updated = await updateTaskUseCase.execute({ id: inProgressTask.id, status: "completed", completedAt: NOT_SPECIFIED });
...
Deno.test("完了→進行中 の状態変更で startedAt を指定しない場合、変更されない(nullにはならない)", async () => {
  ...
  // Given
  const inProgressTask = await createTaskUseCase.execute({ title: "task", status: "completed", due: NOT_SPECIFIED, startedAt: DATE_1, completedAt: DATE_2 });
