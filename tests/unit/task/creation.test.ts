...
import { DATE_1, DATE_2, DATE_3, DATE_4, TASK_ID_1 } from "../../helper.ts";
...
Deno.test("開始日時を持たない状態のタスクに日時を指定してもエラーにならず、開始日時は undefined となる", () => {
  const task = Task.create({
    ...
    title: "task",
    due: UNSPECIFIED,
    startedAt: DATE_2, // unstarted は startedAt を持たない
    createdAt: DATE_1,
    updatedAt: DATE_1
  });
...
Deno.test("開始日時を持たない状態のタスクにUNSPECIFIEDを指定してもエラーにならず、開始日時は undefined となる", () => {
  const task = Task.create({
    ...
    title: "task",
    due: UNSPECIFIED,
    startedAt: UNSPECIFIED, // unstarted は startedAt を持たない
    createdAt: DATE_1,
    updatedAt: DATE_1
  });
