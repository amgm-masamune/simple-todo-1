...
Deno.test("存在するIDを指定するとエラーなく削除でき、IDでタスクを取得できなくなる", async () => {
  const { createTaskUseCase, deleteTaskUseCase, findTaskByIdUseCase } = await createDependencies("in-memory");
...
Deno.test("存在しないIDを指定するとエラーが発生する", async () => {
  const { createTaskUseCase, deleteTaskUseCase } = await createDependencies("in-memory");
